...
class PregledSvihAktivnostiFragment : Fragment(), DatabaseConsumer {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.f05_pregled_svih_aktivnosti, container, false)
    }
    ...
    private fun fetchPodaci(view: View) {
        lifecycleScope.launch {
            try {
                val brojNarudzbi = narudzbaDataSource.dohvatiBrojNarudzbiSvihKorisnika().toInt()
                val brojRezervacijaUsluga = rezervacijaTerminaUslugeDataSource.dohvatiBrojRezervacijaUslugaSvihKorisnika().toInt()
                val brojRezervacijaVeterinara = rezervacijaVeterinaraDataSource.dohvatiBrojRezervacijaVeterinaraSvihKorisnika().toInt()
                val brojWishlist = wishlistDataSource.dohvatiBrojWishlistSvihKorisnika().toInt()
                val brojZahtjeva = zahtjevUdomljavanjeDataSource.dohvatiBrojZahtjevaSvihKorisnika().toInt()
                ...
                view.findViewById<TextView>(R.id.brojWishlistText).text = "Broj wishlist stavki: $brojWishlist"
